	...
	Если МожемЗавершатьРаботу И НеобходимоОжидание Тогда
		...
		Попытка
			...
			Для Каждого ДанныеФайла Из ФайлыКАктуализации Цикл
				SslCI_РаботаСГитом.ОбновитьИзРепозитория(ДанныеФайла.Ссылка, Ложь, ДанныеФайла.Реквизиты);		
			КонецЦикла;
			
			Сообщить(""+ТекущаяДата() + " - "+"Внешние отчеты и обработки обновлены");
		Исключение
			Сообщить("ERROR: "+ТекущаяДата() + ОписаниеОшибки());	
		КонецПопытки;
		// } Подымников (20.12.2022 17:32:44).
