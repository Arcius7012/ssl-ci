...
Процедура SslCI_ОбработкаВыбораПосле(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Справочник.ДополнительныеОтчетыИОбработки.Форма.SslCI_ПереключениеВетки" Тогда
		...
		SslCI_РаботаПоАпи.АктуализацияСправочника(ВыбранноеЗначение);
...
Процедура SslCI_СписокВыборПосле(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.SslCI_ЗадачаВТаскТрекере Тогда
		...
		SslCI_РаботаПоАпи.ОткрытьЗадачуВТрекере(ТекДанные.SslCI_СообщениеКоммита);
	КонецЕсли;
	
	Если Поле = Элементы.SslCI_Коммит Тогда
		СтандартнаяОбработка = Ложь;
		SslCI_РаботаПоАпи.ОткрытьКоммит(ТекДанные.SslCI_ИДКоммита);	
	КонецЕсли;	
...
Процедура ВидимостьРучногоДобавленияФайлов()
	...
	Элементы.СправочникДополнительныеОтчетыИОбработкиSslCI_АктуализацияСправочника.Видимость = СоединениеСГитомУстановлено;
	Элементы.СправочникДополнительныеОтчетыИОбработкиSslCI_ПереключитьВетку.Видимость = СоединениеСГитомУстановлено;
	Элементы.sslСоздать.Видимость = СоединениеСГитомУстановлено;
	Элементы.ГруппаРепозиторий.Видимость = СоединениеСГитомУстановлено;
	Элементы.СписокКонтекстноеМенюsslСоздать.Видимость = СоединениеСГитомУстановлено;
	Элементы.СписокКонтекстноеМенюРепозиторий.Видимость = СоединениеСГитомУстановлено;
