...
Процедура ПриОткрытии(Отказ)
	...
	ДанныеФайла = SslCI_РаботаПоАпи.ИсторияКоммитов(ИмяФайла, Ветка);
...
Процедура КоммитыВыбор(Элемент, ВыбраннаяСтрока, Поле, СтандартнаяОбработка)
	...
	Если Поле = Элементы.КоммитыЗадачаВТаскТрекере Тогда
		SslCI_РаботаПоАпи.ОткрытьЗадачуВТрекере(ТекДанные.СообщениеКоммита);
	КонецЕсли;
	
	Если Поле = Элементы.КоммитыКоммит Тогда
		SslCI_РаботаПоАпи.ОткрытьКоммит(ТекДанные.ИдКоммита);	
	КонецЕсли;
...
Процедура ПерейтиНаКоммит(Команда)
	...
	Если ТекДанные <> Неопределено Тогда
		SslCI_РаботаПоАпи.ПерейтиНаКоммит(Файл, ИмяФайла, ТекДанные.ИдКоммита);
