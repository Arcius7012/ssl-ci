...
Процедура SslCI_ОбработкаВыбораПосле(ВыбранноеЗначение, ИсточникВыбора)
	...
	Если ИсточникВыбора.ИмяФормы = "Справочник.ДополнительныеОтчетыИОбработки.Форма.SslCI_ВыборФайлаИзРепозитория" Тогда
		...
		Иначе
			...
			ДанныеФайла 	= SslCI_РаботаСГитом.ПолучитьФайлИзРепозитория(Объект.SslCI_АдресВРепозитории, Объект.SslCI_ИмяВетки);
			ДанныеКоммита 	= SslCI_РаботаСГитом.ДанныеКоммита(ДанныеФайла.СтруктураФайла.last_commit_id);			
			ОписаниеКоммита = SslCI_РаботаСГитом.ОписаниеКоммита(ДанныеКоммита.СтруктураФайла);
...
Процедура SslCI_ВыборФайлаПродолжение(Результат, ДопПараметры) Экспорт
	...
	Если Результат = КодВозвратаДиалога.Да Тогда
		Записать();
		SslCI_РаботаСГитом.ОбновитьИзРепозитория(Объект.Ссылка, Истина);
		Закрыть();
	КонецЕсли;
...
Процедура SslCI_ЗадачаВТаскТрекереНажатиеВместо(Элемент)
	SslCI_РаботаСГитом.ОткрытьЗадачуВТрекере(Объект.SslCI_СообщениеКоммита);
КонецПроцедуры

&НаКлиенте
Процедура SslCI_КоммитНажатиеВместо(Элемент)
	SslCI_РаботаСГитом.ОткрытьКоммит(Объект.SslCI_ИДКоммита);
КонецПроцедуры
